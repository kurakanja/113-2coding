...
def main():
    ...
    def score():
        nonlocal eaten_study, eaten_cactus, eaten_bird

        nonlocal eaten_study, eaten_cactus, eaten_bird
        text_color = (94, 94, 94)

        # 分數
        text = font.render("分數：" + str(points), True, text_color)
        screen.blit(text, (950, 20))

        # 類別統計（中文顯示）
        study_text = font.render(f"study：{eaten_study}", True, text_color)
        cactus_text = font.render(f"cat：{eaten_cactus}", True, text_color)
        bird_text = font.render(f"bird：{eaten_bird}", True, text_color)

        screen.blit(study_text, (800, 60))
        screen.blit(cactus_text, (800, 90))
        screen.blit(bird_text, (800, 120))
    ...
    while run:
        ...
        if random.random() < 0.2:
            obstacle_type = random.choice(["studying", "large", "bird"])
